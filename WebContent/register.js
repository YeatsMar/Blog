...
function validate() {
	...
	var accountPattern = /^\w+$/;// pattern for email
	// 在前台验证
	if (account.value === "") {
		warn0.innerHTML = "账号不得为空！";
        return false;
	} else if (account.value.length < 2 || account.value.length > 14) {
		warn0.innerHTML = "账号长度控制在2-14个字符内！";
        return false;
	} else if (!accountPattern.test(account.value)) {
		warn0.innerHTML = "账号只能包含英文字母、数字和下划线！";
        return false;
	}
	else {
		warn0.innerHTML = "√";
	...
	if (password.value === "") {
		...
		return false;
	} else if (!passwordPattern1.test(password.value)) {
		warn2.innerHTML = "密码只能包含英文字母、数字和下划线！";
		return false;
	} else if (!passwordPattern2.test(password.value)) {// show that there are
		// only numbers//must
		// include letter
		warn2.innerHTML = "密码不得为纯数字！";
		return false;
	} else if (password.value.length <= 6) {
		warn2.innerHTML = "密码长度必须超过6个字符！";
		return false;
	} else {
		warn2.innerHTML = "√";
	}
